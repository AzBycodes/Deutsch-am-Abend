...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (courseParam) {
        ...
        if (courseSelect) {
            const option = courseSelect.querySelector(`option[value="${courseParam}"]`);
            if (option) {
...
function handleFormSubmit(e) {
    ...
    // Create mailto link
    const mailtoLink = `mailto:nuralleangebote@gmail.com?subject=Course Registration - ${data.firstName} ${data.lastName}&body=${encodeURIComponent(emailBody)}`;
...
function createEmailBody(data, courseName) {
    ...
    const germanLevelText = germanLevelSelect.options[germanLevelSelect.selectedIndex].text;
    body += `${isEnglish ? 'Current German Level' : 'Aktuelles Deutschniveau'}: ${germanLevelText}\n`;
    
    const previousCourses = data.previousCourses === 'yes' 
        ? (isEnglish ? 'Yes' : 'Ja')
        : (isEnglish ? 'No' : 'Nein');
    ...
    const paymentPlanText = paymentPlanSelect.options[paymentPlanSelect.selectedIndex].text;
    ...
    const newsletter = data.newsletter === 'on'
        ? (isEnglish ? 'Yes, subscribed to newsletter' : 'Ja, Newsletter abonniert')
        : (isEnglish ? 'No newsletter subscription' : 'Kein Newsletter-Abonnement');
...
function showConfirmation() {
    ...
    const message = isEnglish
        ? 'Your registration form is ready to send!\n\nYour email client should open shortly. If it doesn\'t open automatically, please send an email to nuralleangebote@gmail.com with your registration details.\n\nYou will receive a confirmation within 24 hours.'
        : 'Ihr Anmeldeformular ist versandbereit!\n\nIhr E-Mail-Client sollte sich in Kürze öffnen. Falls er sich nicht automatisch öffnet, senden Sie bitte eine E-Mail an nuralleangebote@gmail.com mit Ihren Anmeldedaten.\n\nSie erhalten innerhalb von 24 Stunden eine Bestätigung.';
